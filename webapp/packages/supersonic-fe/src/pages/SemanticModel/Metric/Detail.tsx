...
import { SENSITIVE_LEVEL_ENUM, SENSITIVE_LEVEL_COLOR,theme_body_bg_color,head_color,text_color } from '../constant';
...
const DescriptionItem = ({ title, content, icon }: DescriptionItemProps) => (
  <Tooltip title={title}>
    <div style={{ width: 'max-content', fontSize: 14, color: '#7789a3' }}>
      <Space>
...
const MetricDetail: React.FC<Props> = ({ domainManger }) => {
  ...
  const contentStyle: React.CSSProperties = {
    minHeight: 120,
    color: '#fff',
    backgroundColor: theme_body_bg_color,
  };
                ...
                <Space>
                  ...
                  {metircData?.name && (
                    <>
                      ...
                      <span style={{ fontSize: 16, color: text_color }}>{metircData?.bizName}</span>
                    </>
                  )}

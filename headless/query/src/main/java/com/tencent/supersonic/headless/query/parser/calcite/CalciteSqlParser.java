...
public class CalciteSqlParser implements SqlParser {
    ...
    @Override
    public QueryStatement simpleExplain(QueryStatement queryStatement, AggOption isAgg, Catalog catalog) throws Exception {
        MetricReq metricReq = queryStatement.getMetricReq();
        HeadlessModel semanticModel = headlessSchemaManager.get(metricReq.getRootPath());
        if (semanticModel == null) {
            queryStatement.setErrMsg("semanticSchema not found");
            return queryStatement;
        }
        queryStatement.setMetricReq(metricReq);
        HeadlessSchema semanticSchema = getSemanticSchema(semanticModel);
